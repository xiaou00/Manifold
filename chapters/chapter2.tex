...
\begin{proof}
    ...
    \end{align*}
    对所有分量成立, 从而对导数向量 $\dfrac{\d}{\d t}(\phi\circ\gamma)\Big|_{t=0}$ 成立. 因此 $[\gamma_1]=[\gamma_2]$. 这说明 $\theta$ 是单射. 另一方面, 对于任意 $v\in T_pM$, 我们仍然可以选取一个包含 $p$ 的容许坐标卡 $(U,\phi)$, 其中 $\phi=(\phi^1,\ldots,\phi^m)$, 则对于任意 $i=1,\ldots,m$, 都有 $v(\phi^i)\in\R$. 因此我们可以定义
\end{proof}